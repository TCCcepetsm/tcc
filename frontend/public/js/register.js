document.addEventListener('DOMContentLoaded', () => {
    ...
    if (localStorage.getItem('authToken')) {
        ...
        const redirectUrl = userInfo?.roles?.includes('ROLE_ADMIN')
            ? '/frontend/views/inicialAdmin.html'
            : '/inicial.html';
...
async function handleResponse(response, isEmpresa) {
    ...
    if (isEmpresa) {
        ...
        setTimeout(() => window.location.href = `${baseUrl}/inicialAdmin.html`, 2000);
    } else {
        setTimeout(() => window.location.href = `${baseUrl}/views/login.html`, 2000);
    }
}
